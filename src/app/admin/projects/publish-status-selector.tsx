...
export function PublishStatusSelector({ projectId, isPublished }: { projectId: string, isPublished: boolean }) {
  ...
  return <SingleBadgeSelector
    options={[
      { value: "yes", label: () => <Badge className="bg-green-300">เผยแพร่แล้ว</Badge> },
      { value: "no", label: () => <Badge className="bg-neutral-200">ไม่</Badge> },
    ]}
    onValueChange={async (newStatus) => {
      try {
        await setProjectPublished.mutateAsync({ projectId, isPublished: newStatus === "yes" });
        toast({
